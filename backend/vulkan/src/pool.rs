...
impl pool::RawCommandPool<Backend> for RawCommandPool {
    fn reset(&mut self) {
        self.next_buffer = 0;
        unsafe {
            self.device.0.fp_v1_0().reset_command_pool(self.device.0.handle(),
                                                       self.pool,
    ...
    unsafe fn acquire_command_buffer<'a>(&'a mut self) -> &mut CommandBuffer {
        ...
        self.device.0.begin_command_buffer(buffer.raw, &info).unwrap(); // TODO: error handling
        buffer
    }
        ...
        where Q: AsRef<CommandQueue>
    {
        ...
        let command_buffers = queue.device
            .0
            .allocate_command_buffers(&info)
            .expect("Error on command buffer allocation"); // TODO: better error handling
        ;
        let command_buffers = command_buffers.into_iter()
            .map(|buffer| {
                CommandBuffer {
                    raw: buffer,
