...
pub fn main() {
    ...
    let gl_builder = gfx_window_glutin::config_context(gl_builder,
    ...
    let gfx::Gpu { mut device, mut graphics_queues, .. } = adapters[0].open_with(|family, ty| {
    ...
    let pso = device.create_pipeline_simple(include_bytes!("shader/quad_120.glslv"),
                                include_bytes!("shader/quad_120.glslf"),
                                pipe::new())
        .unwrap();
    let (vertex_buffer, slice) =
        device.create_vertex_buffer_with_slice(&QUAD, &[0u16, 1, 2, 2, 3, 0] as &[u16]);
    ...
    while running {
        ...
        // Draw a frame
        {
            ...
            encoder.synced_flush(&mut graphics_queue,
                              &[&frame_semaphore],
                              &[&draw_semaphore],
                              Some(&frame_fence))
