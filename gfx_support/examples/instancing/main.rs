// Copyright 2017 The gfx developers.
//
...
impl<B: gfx::Backend> Application<B> for App<B> {
    fn new(device: &mut B::Device,
        ...
        let instances = device.create_buffer(instance_count as usize,
        ...
        let (quad_vertices, mut slice) =
            device.create_vertex_buffer_with_slice(&QUAD_VERTICES, &QUAD_INDICES[..]);
        slice.instances = Some((instance_count, 0));
        let locals = Locals { scale: size };

        App {
            pso: device.create_pipeline_simple(vs.select(backend).unwrap(),
            ...
            data: pipe::Data {
                ...
                instance: instances,
                scale: size,
                locals: device.create_buffer_immutable(&[locals],
